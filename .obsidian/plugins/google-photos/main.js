...
var PhotosApi = class {
  ...
  async listAlbums() {
    ...
    if (sharedAlbums == null ? void 0 : sharedAlbums.sharedAlbums) {
...
var GooglePhotosSettingTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian5.Setting(containerEl).setName("Photos API").setHeading();
    ...
    new import_obsidian5.Setting(containerEl).setDesc("Google Photos will automatically authenticate you when you start using the plugin. You can also manually initiate the authentication process by clicking this button.").addButton((btn) => btn.setButtonText("Open Photos API auth").setCta().onClick(async () => {
      await this.plugin.oauth.authenticate();
    }));
    new import_obsidian5.Setting(containerEl).setName("Thumbnail settings").setHeading().setDesc("Set the maximum size for your locally-saved thumbnail image. The image will fit within these dimensions while keeping the original aspect ratio.");
    ...
    new import_obsidian5.Setting(containerEl).setName("Date settings").setHeading();
    new import_obsidian5.Setting(containerEl).setName("Default to limiting shown photos to date(s)").setDesc("Choose to limit the photos shown in popup modal to a specific date(s) determined by the option below.").addToggle((toggle) => {
    ...
    if (this.plugin.settings.getDateFrom === "Note's title" /* NOTE_TITLE */) {
      new import_obsidian5.Setting(containerEl).setName("Title date format").addText((text) => text.setPlaceholder(DEFAULT_SETTINGS.getDateFromFormat).setValue(this.plugin.settings.getDateFromFormat).onChange(async (value) => {
      ...
      new import_obsidian5.Setting(containerEl).setName("Front matter key").addText((text) => text.setPlaceholder(DEFAULT_SETTINGS.getDateFromFrontMatterKey).setValue(this.plugin.settings.getDateFromFrontMatterKey).onChange(async (value) => {
      ...
      new import_obsidian5.Setting(containerEl).setName("Front matter date format").addText((text) => text.setPlaceholder(DEFAULT_SETTINGS.getDateFromFormat).setValue(this.plugin.settings.getDateFromFormat).onChange(async (value) => {
        this.plugin.settings.getDateFromFormat = value.trim();
        await this.plugin.saveSettings();
      })).then((setting) => {
        setting.descEl.appendText("This is the ");
    ...
    new import_obsidian5.Setting(containerEl).setName("Show photos in range of days?").setDesc("Enable to show photos from a range of days before and after the note date.").addToggle((toggle) => {
    ...
    if (this.plugin.settings.showPhotosInDateRange) {
      new import_obsidian5.Setting(containerEl).setName("Number of days in the past").setDesc("Number of days in the past to show photos from.").addText((text) => {
      ...
      new import_obsidian5.Setting(containerEl).setName("Number of days in the future").setDesc("Number of days in the future to show photos from.").addText((text) => {
        ...
        try {
          ...
          if (e === "Retry") {
            ...
            if (e === "Unauthenticated") {
              this.active = false;
              new import_obsidian6.Notice("Failed to authenticate");
            } else {
              targetEl.createEl("p", {
                text: e
              });
...
// src/suggesters/AlbumSuggest.ts
var import_obsidian8 = require("obsidian");
var AlbumSuggest = class extends import_obsidian8.SuggestModal {
  constructor(plugin) {
    super(plugin.app);
    this.plugin = plugin;
  ...
  renderSuggestion(item, el) {
    const sharedAlbum = item.isSharedAlbum ? " - \u{1F465} shared album" : "";
  ...
  parseContents() {
    ...
    if (source === "today") {
      this.searchParams = {
        filters: {
          dateFilter: {
            dates: [dateToGoogleDateFilter((0, import_obsidian9.moment)())]
          }
        }
      };
...
// src/main.ts
var GooglePhotos = class extends import_obsidian10.Plugin {
  async onload() {
  ...
  getNoteDate(file) {
    if (this.settings.getDateFrom === "Note's title" /* NOTE_TITLE */) {
      return (0, import_obsidian10.moment)(file.basename, this.settings.getDateFromFormat, true);
      ...
      if (frontMatter && frontMatter[this.settings.getDateFromFrontMatterKey]) {
        return (0, import_obsidian10.moment)(frontMatter[this.settings.getDateFromFrontMatterKey], this.settings.getDateFromFormat, true);
      } else {
        return (0, import_obsidian10.moment)("invalid date");
      }
    } else {
      return (0, import_obsidian10.moment)();
    }
  }
};
/*!
